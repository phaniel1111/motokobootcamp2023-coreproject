<script>
  ...
  async function vote(thisid, votepayload) {
    ...
    var obj = {};
    obj[votepayload] = null;
    let require = {vote:  obj,  proposal_id: BigInt(thisid)};
    let res = await dao.vote(require)
    if (res.Ok) {
      return res.Ok
    } else {
      ...
      {:then res}
        <div class="votingdiv">
          ...
          <div>
            ...
            <button on:click={() => handleVoteClick("yes")}>Yes</button>
            <button on:click={() => handleVoteClick("no")}>No</button>
            {#if $hasvoted === true}
              {#await promise}
                <h1 class="slogan">Loading...</h1>
              {:then res2}
