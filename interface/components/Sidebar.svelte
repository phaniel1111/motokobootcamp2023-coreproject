...
<nav class:expanded={isExpanded} on:mouseover={() => mouseover()}>
  <ul>
    {#if isExpanded}
      ...
      <li on:click={() => handleView()}>🚀 View Proposal</li>
      <li on:click={() => handleCreate()}>⭐️ Create Proposal</li>
      <li on:click={() => handleVote()}>🗑 Vote</li>
      <li on:click={() => handleCreateN()}>🗑 Create Neuron</li>
    {/if}
  </ul>
</nav>
